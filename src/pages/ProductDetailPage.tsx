...
export default function ProductDetailPage() {
  ...
  async function addCartItem(productId: number) {
    ...
    await axios.post(
      'http://172.16.37.21:8000/api/cart-items',
  ...
  async function removeCartItem(productId: number) {
    ...
    await axios.delete(`http://172.16.37.21:8000/api/cart-items/remove/${productId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  }
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await axios.get(`http://172.16.37.21:8000/api/produk/${id}`);
    ...
    const fetchDiskusi = async () => {
      try {
        const res = await axios.get(`http://172.16.37.21:8000/api/produk/${id}/diskusi`);
  ...
  const handleSubmitDiskusi = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post(
        `http://172.16.37.21:8000/api/produk/${id}/diskusi`,
        {

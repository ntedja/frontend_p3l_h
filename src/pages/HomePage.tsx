...
const categories = [
  ...
  { label: "Perlengkapan Bayi & Anak", value: "Bayi & Anak", icon: "bi-emoji-smile", slug: "bayi-anak" },
  { label: "Otomotif & Aksesoris", value: "Otomotif", icon: "bi-car-front", slug: "otomotif" },
  { label: "Perlengkapan Taman & Outdoor", value: "Taman & Outdoor", icon: "bi-flower2", slug: "taman-outdoor" },
  { label: "Peralatan Kantor & Industri", value: "Kantor & Industri", icon: "bi-briefcase", slug: "kantor-industri" },
    ...
    axios.get(`${API_BASE_URL}/produk`)
      .then((res) => {
        const products = res.data.map((item: any) => {
          const catObj = categories.find(c => c.value.toLowerCase() === item.category.toLowerCase());
          return {
            id: item.id,
            name: item.name,
            price: item.price,
